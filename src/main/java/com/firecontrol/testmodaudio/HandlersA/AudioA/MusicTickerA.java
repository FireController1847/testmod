...
public class MusicTickerA implements ITickable {
	private final Random rand = new Random();
	private final Minecraft mc;
	private ISoundA currentMusic;
	private int timeUntilNextMusic = 100;

	public MusicTickerA(Minecraft mcIn) {
		this.mc = mcIn;
	}
	...
	public void update() {
		...
		if (this.currentMusic != null) {
			if (!musicticker$musictype.getMusicLocation().getSoundName().equals(this.currentMusic.getSoundLocation())) {
				TestModA.soundHandler.stopSound(this.currentMusic);
				this.timeUntilNextMusic = MathHelper.getInt(this.rand, 0, musicticker$musictype.getMinDelay() / 2);
			}

			if (!TestModA.soundHandler.isSoundPlaying(this.currentMusic)) {
	...
	public void playMusic(MusicTicker.MusicType requestedMusicType) {
		this.currentMusic = PositionedSoundRecordA.getMusicRecord(requestedMusicType.getMusicLocation());
		TestModA.soundHandler.playSound(this.currentMusic);
